...
email:
  api_key_env: ANCHOVY_RESEND_API_KEY       # → RESEND_API_KEY for this site
```
...
- `email.api_key_env` binds a source → `RESEND_API_KEY` for any site feature
  that sends email (contact forms, order confirmations, shipping notices).
