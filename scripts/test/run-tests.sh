...
assert_contains   "prose images are responsive" ".c-prose img" "$BUNDLE"
assert_contains   "prose images stay within narrow screens" "max-width: 100%" "$BUNDLE"
assert_contains   "prose images preserve aspect ratio" "height: auto" "$BUNDLE"
...
assert_not_contains "unused playful font is not loaded" 'family=Baloo' "$GOAL_HTML"
...
assert_contains "hero accepts image_position above" 'c-hero--image-above' "$AWARD_HTML"
assert_contains "hero above/below stacking CSS is present" '.c-hero--image-above .c-hero__media' "$(cat scaffold/src/css/components.css)"
